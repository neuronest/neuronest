...
from service_evaluator.api.dependencies import (
    ...
    use_google_application_credentials,
    use_job_prefix_name,
    use_project_id,
    use_region,
    use_serialized_service_client_parameters,
    use_service_name,
...
def _handle_already_running_jobs_response(
    response: Response,
    firestore_client: FirestoreClient,
    firestore_jobs_collection: str,
    service_name: EvaluatedServiceName,
...
def evaluate(
    ...
    serialized_service_client_parameters: str = Depends(
        use_serialized_service_client_parameters
    ),
):
    ...
    if len(running_jobs) > 0:
        return _handle_already_running_jobs_response(
            response=response,
            firestore_client=firestore_client,
            firestore_jobs_collection=firestore_jobs_collection,
            service_name=service_name,
    ...
    service_predictor = ServicePredictorFactory.new(
        service_name=service_name,
        service_client_parameters=json.loads(serialized_service_client_parameters),
    )
